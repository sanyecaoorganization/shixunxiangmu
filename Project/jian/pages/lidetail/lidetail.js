...
Page({
  ...
  hunDian:function(e){
    ...
    wx.navigateTo({
      url: '/pages/lifujs/lifujs?shopid='+$data.shopid+"&shopname="+$data.shopname+"&shopaddress="+$data.shopaddress+"&weidu="+$data.weidu+"&jingdu="+$data.jingdu
    })
    ...
    wx.request({
      ...
      success:res=>{
        ...
        for(var i=0;i<res.data.filter(item => item.shopid == options.shopid && item.styleid == options.styleid).length;i++){
          if(options.id==res.data.filter(item => item.shopid == options.shopid && item.styleid == options.styleid)[i].pnum){
            this.setData({
              current:i,
              swiperCurrent:i,
              // images:[this.data.now[i].pimage1,this.data.now[i].pimage2,this.data.now[i].pimage3,this.data.now[i].pimage4,this.data.now[i].pimage5,this.data.now[i].pimage6,this.data.now[i].pimage7]
            })
            console.log(this.data.swiperCurrent)
          }
          // console.log(options.id)
  ...
  sc:function(e){
    ...
    if(that.data.uname){
      wx.request({
        ...
        success:res=>{
          that.setData({
            nownum:res.data.filter(item => item.shopid == that.data.shid && item.styleid == that.data.stid)[e.currentTarget.dataset.nowid].pnum
          })
          console.log(that.data.nownum)  
          // console.log(res.data.filter(item => item.pnum == that.data.nownum)[0])
          var img = "collect.image"
          that.setData({
            [img]:res.data.filter(item => item.pnum == that.data.nownum)[0].pimage1
          });
          var idd = "collect.id"
          that.setData({
          ...
          that.setData({
            [typeidd]:res.data.filter(item => item.pnum == that.data.nownum)[0].typeid
          });
          var mn = "collect.name"
          that.setData({
            [mn]:res.data.filter(item => item.pnum == that.data.nownum)[0].name
          });
